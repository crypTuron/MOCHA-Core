...
CzMOCHATracker::CzMOCHATracker(std::string strWalletFile)
{
...
CzMOCHATracker::~CzMOCHATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMOCHATracker::Init()
{
...
bool CzMOCHATracker::Archive(CMintMeta& meta)
{
...
bool CzMOCHATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMOCHATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMOCHATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMOCHATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMOCHATracker::GetSerialHashes()
{
...
CAmount CzMOCHATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMOCHATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMOCHATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMOCHATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMOCHATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMOCHATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMOCHATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMOCHATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMOCHATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMOCHATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMOCHATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMOCHATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMOCHATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMOCHATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMOCHATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMOCHATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMOCHATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzmocha from DB\n", __func__, listDeterministicDB.size());
    }
