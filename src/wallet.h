...
// Possible states for zMOCHA send
enum ZerocoinSpendStatus {
    ZMOCHA_SPEND_OKAY = 0,                            // No error
    ZMOCHA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMOCHA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMOCHA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMOCHA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMOCHA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMOCHA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMOCHA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMOCHA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMOCHA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMOCHA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMOCHA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMOCHA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMOCHA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMOCHA_SPENT_USED_ZMOCHA = 14,                      // Coin has already been spend
    ZMOCHA_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZMOCHA_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZMochaBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZMOCHAOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmochaAuto) const;
    ...
    */
    ...
    CzMOCHAWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzMOCHATracker> zmochaTracker;
    ...
    void setZWallet(CzMOCHAWallet* zwallet)
    {
        zwalletMain = zwallet;
        zmochaTracker = std::unique_ptr<CzMOCHATracker>(new CzMOCHATracker(strWalletFile));
    }

    CzMOCHAWallet* getZWallet() { return zwalletMain; }
    ...
    void setZMochaAutoBackups(bool fEnabled)
    {
