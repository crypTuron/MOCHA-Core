...
#ifndef MOCHACHAIN_ZMOCHAWALLET_H
#define MOCHACHAIN_ZMOCHAWALLET_H
...
public:
    CzMOCHAWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMOCHA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMOCHA(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //MOCHACHAIN_ZMOCHAWALLET_H
