...
CzMOCHAWallet::CzMOCHAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmocha seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMOCHASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMOCHASeed_deprecated()) {
                    LogPrintf("%s: Updated zMOCHA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmocha seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmocha wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMOCHAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMOCHACount(nCountLastUsed);
    else if (!walletdb.ReadZMOCHACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMOCHAWallet::Lock()
{
    seedMaster = 0;
}

void CzMOCHAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMOCHAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMOCHA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMOCHAWallet::LoadMintPoolFromDB()
{
...
void CzMOCHAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMOCHAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMOCHAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmochaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMOCHAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMOCHA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmochaTracker which also adds to database
    pwalletMain->zmochaTracker->Add(dMint, true);
...
void CzMOCHAWallet::SeedToZMOCHA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMOCHAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMOCHAWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZMOCHACount(nCountLastUsed);
...
void CzMOCHAWallet::GenerateDeterministicZMOCHA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMOCHAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMOCHAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
