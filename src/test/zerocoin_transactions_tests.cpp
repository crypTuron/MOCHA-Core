...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zmochaTracker = unique_ptr<CzMOCHATracker>(new CzMOCHATracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZMOCHA_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZMOCHA_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
