...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMochaChainAmount"))
        settings.setValue("nAnonymizeMochaChainAmount", 1000);

    nAnonymizeMochaChainAmount = settings.value("nAnonymizeMochaChainAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMochaChainAmount"))
        SoftSetArg("-anonymizemochachainamount", settings.value("nAnonymizeMochaChainAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mochachain.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMochaChainAmount:
            return QVariant(nAnonymizeMochaChainAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMochaChainAmount:
            nAnonymizeMochaChainAmount = value.toInt();
            settings.setValue("nAnonymizeMochaChainAmount", nAnonymizeMochaChainAmount);
            emit anonymizeMochaChainAmountChanged(nAnonymizeMochaChainAmount);
            break;
        case CoinControlFeatures:
