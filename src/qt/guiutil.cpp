...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MochaChain address (e.g. %1)").arg("MYAQChamL2LxZgWeNn5Mo8ggsVZcNXCPPc"));
#endif
...
{
    // return if URI is not valid or is no MochaChain: URI
...
{
    // Convert mochachain:// to mochachain:
    //
    //    Cannot handle this later, because mochachain:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open mochachain.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MochaChain.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MochaChain.lnk
...
{
    return GetAutostartDir() / "mochachain.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a mochachain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MochaChain\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the mochachain app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add mochachain app to startup item list
