...
#include "ui_zmochacontroldialog.h"
...
std::set<std::string> ZMochaControlDialog::setSelectedMints;
std::set<CMintMeta> ZMochaControlDialog::setMints;

ZMochaControlDialog::ZMochaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMochaControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZMochaControlDialog::~ZMochaControlDialog()
{
    delete ui;
}

void ZMochaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMochaControlDialog::updateList()
{
...
void ZMochaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMochaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZMocha_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMochaControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZMochaControlDialog::GetSelectedMints()
{
...
void ZMochaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
