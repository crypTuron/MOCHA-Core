                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zMOCHApayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zMOCHA ought to be enough for anybody." - Bill Gates, 2017
    ui->zMOCHApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMOCHASyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMOCHApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMOCHA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMOCHA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMOCHA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMOCHA in ") +
...
void PrivacyDialog::on_pushButtonSpendzMOCHA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMOCHA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMOCHA
        sendzMOCHA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMOCHA
    sendzMOCHA();
}

void PrivacyDialog::on_pushButtonZMochaControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZMochaControlDialog* zMochaControl = new ZMochaControlDialog(this);
    zMochaControl->setModel(walletModel);
    zMochaControl->exec();
}

void PrivacyDialog::setZMochaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMOCHASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMOCHA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid MochaChain Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMOCHApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMOCHA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MOCHA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMOCHA</b>";
    ...
    // use mints from zMOCHA selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMochaControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZMochaControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMOCHA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zMOCHA"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZMOCHA_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zMOCHA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zMOCHA"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zMOCHA was spent successfully update the addressbook with the label
    ...
    // Clear zmocha selector in case it was used
    ZMochaControlDialog::setSelectedMints.clear();
    ui->labelzMOCHASelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMOCHA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Mocha, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMOCHA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zmochaTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMOCHA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMOCHA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMOCHA "));
    ui->labelzMOCHAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMOCHASyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzMOCHA->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zMOCHA
        ui->pushButtonMintzMOCHA->setEnabled(false);
        ui->pushButtonMintzMOCHA->setToolTip(tr("zMOCHA is currently disabled due to maintenance."));

        // Spend zMOCHA
        ui->pushButtonSpendzMOCHA->setEnabled(false);
        ui->pushButtonSpendzMOCHA->setToolTip(tr("zMOCHA is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zMOCHA
        ui->pushButtonMintzMOCHA->setEnabled(true);
        ui->pushButtonMintzMOCHA->setToolTip(tr("PrivacyDialog", "Enter an amount of MOCHA to convert to zMOCHA", 0));

        // Spend zMOCHA
        ui->pushButtonSpendzMOCHA->setEnabled(true);
        ui->pushButtonSpendzMOCHA->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}