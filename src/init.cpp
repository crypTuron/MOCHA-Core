...
#include "config/mochachain-config.h"
#endif
...
CzMOCHAWallet* zwalletMain = NULL;
...
void PrepareShutdown()
{
    ...
    RenameThread("mochachain-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "mochachain.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "mochachaind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MOCHA and zMOCHA money supply statistics") + " " + _("on startup"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, mochachain, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MochaChain specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzmocha=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMocha minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmochabackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zMOCHA backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizemochachainamount=<n>     " + strprintf(_("Keep N MOCHA anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i MochaChain LLC"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("mochachain-loadblk");
...
/** Sanity checks
 *  Ensure that MochaChain is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MochaChain Core is shutting down."));
...
#endif
    // Make sure only a single MochaChain process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MochaChain Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MochaChain version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MochaChain specific: zerocoin and spork DB's
                ...
                // MochaChain: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZMOCHAMinted();
                        RecalculateZMOCHASpent();
                    }
                    RecalculateMOCHASupply(1);
                }
                ...
                // MochaChain: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MochaChain Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MochaChain Core to complete") << "\n";
        ...
        zwalletMain = new CzMOCHAWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zMOCHA wallet..."));

        bool fEnableZMochaBackups = GetBoolArg("-backupzmocha", true);
        pwalletMain->setZMochaAutoBackups(fEnableZMochaBackups);
        ...
        pwalletMain->zmochaTracker->Init();
...
//    }
//
//    nAnonymizeMochaChainAmount = GetArg("-anonymizemochachainamount", 0);
//    if (nAnonymizeMochaChainAmount > 999999) nAnonymizeMochaChainAmount = 999999;
//    if (nAnonymizeMochaChainAmount < 2) nAnonymizeMochaChainAmount = 2;
    ...
    LogPrintf("Anonymize MochaChain Amount %d\n", nAnonymizeMochaChainAmount);
    ...
    /* Denominations
       ...
       For example:
       1MOCHA+1000 == (.1MOCHA+100)*10
       10MOCHA+10000 == (1MOCHA+1000)*10
    */
